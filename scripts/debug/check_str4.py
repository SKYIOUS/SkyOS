with open('C:\\Users\\nanda\\Desktop\\Github\\SkyOS\\target\\x86_64-sarga\\release\\init', 'rb') as f:
    ...
    for part in [b'/etc', b'init.cfg', b'etc/init']:
        pos = data.find(part)
        if pos >= 0:
            print(f'Partial "{part.decode()}" at offset 0x{pos:x}: {data[max(0,pos-4):pos+len(part)+4]}')
        else:
    ...
    for i in range(len(data) - 20):
        if data[i] == 0x0D and data[i:i+14] == b'\x0D/etc/init.cfg':  # 13-char string with length prefix
            print(f'[LEN13] /etc/init.cfg at 0x{i:x}')
        if data[i] == 14 and data[i:i+15] == b'\x0E/etc/init.cfg\x00':  # null terminated with length
    ...
    for area_start in [0x3468, 0x3600]:
        ...
        for j in range(0, min(len(area), 0x200), 16):
            ...
            ascii_str = ''.join(chr(b) if 32 <= b < 127 else '.' for b in area[j:j+16])
