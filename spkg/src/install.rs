...
pub fn download_package(repo: &RepoConfig, entry: &RepoIndexEntry) -> Result<Vec<u8>, &'static str> {
...
fn mkparent(path: &str) {
    if let Some(slash) = path.rfind('/') {
        ...
        if parent.len() > 1 { ensure_dir(parent); }
    }
}
...
fn write_file_at(path: &str, data: &[u8]) -> Result<(), &'static str> {
    ...
    if (fd as i64) < 0 { return Err("cannot create file"); }
    let _ = unsafe { libsarga::syscall::syscall3(1, fd as u64, data.as_ptr() as u64, data.len() as u64) };
...
pub fn extract_tar(data: &[u8]) -> Result<Vec<String>, &'static str> {
    ...
    while off + BLOCK <= data.len() {
        ...
        if magic != "ustar" { return Err("invalid tar archive"); }
...
pub fn install_package(data: &[u8], entry: &RepoIndexEntry, db: &mut Vec<InstalledEntry>) -> Result<(), &'static str> {
    ...
    let deps = entry.dependencies.iter().map(|d| {
...
pub fn remove_package(entry: &InstalledEntry, db: &mut Vec<InstalledEntry>) -> Result<(), &'static str> {
...
pub fn fetch_cached_spkg(_repo_name: &str, entry: &RepoIndexEntry) -> Result<Vec<u8>, &'static str> {
