...
fn main_test() -> i32 {
    ...
    if fd < 0 {
        println!("FAIL: could not create /test_file ({})", fd);
        return 1;
    }
    ...
    if fd_ro < 0 {
        println!("FAIL: O_RDONLY open returned {}", fd_ro);
        failures += 1;
    } else {
        let w = raw::write(fd_ro, b"x");
        if w >= 0 {
            println!("FAIL: write on O_RDONLY succeeded");
            failures += 1;
        } else {
            println!("PASS: write on O_RDONLY denied");
        }
        raw::close(fd_ro);
    }
    ...
    if fd_wo < 0 {
        println!("FAIL: O_WRONLY open returned {}", fd_wo);
        failures += 1;
    } else {
        let mut tmp = [0u8; 4];
        let r = raw::read(fd_wo, &mut tmp);
        if r >= 0 {
            println!("FAIL: read on O_WRONLY succeeded");
            failures += 1;
        } else {
            println!("PASS: read on O_WRONLY denied");
        }
        raw::close(fd_wo);
    }
    ...
    if r < 0 {
        println!("WARN: setuid(1000) returned {}", r);
    }
