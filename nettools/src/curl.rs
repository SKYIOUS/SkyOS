...
fn user_main() -> i32 {
    ...
    let (host, port) = if host_str.starts_with('[') {
        ...
        let p = host_str[close + 1..].strip_prefix(':').and_then(|s| s.parse().ok()).unwrap_or(80);
        (addr, p)
    } else {
        let parts: Vec<&str> = host_str.split(':').collect();
        (parts[0], if parts.len() > 1 { parts[1].parse().unwrap_or(80) } else { 80 })
    };
        ...
        match net::parse_ipv6(raw) {
            Some(ip6) => {
                let fd = match net::socket(net::AF_INET6, net::SOCK_STREAM, 0) {
                    Ok(f) => f, Err(e) => { println!("curl: IPv6 socket: {}", e); return 1; }
                };
                ...
                match net::connect(fd, addr.as_bytes()) {
                    Ok(_) => {
                        ...
                        loop {
                            match net::recv(fd, &mut buf) {
                                Ok(0) => break, Ok(n) => { io::write_all(1, &buf[..n]).ok(); }
                                Err(_) => break,
                            }
                        }
                        let _ = io::close(fd);
                    ...
                    Err(e) => { println!("curl: IPv6 connect failed: {}", e); return 1; }
                }
            }
            None => { println!("curl: invalid IPv6 address"); return 1; }
        }
    }

    if host_str.starts_with('[') { return 0; }
    ...
    match net::socket(net::AF_INET, net::SOCK_STREAM, 0) {
        Ok(fd) => {
            ...
            match net::connect(fd, addr.as_bytes()) {
                ...
                Err(e) => { println!("curl: connect failed: {}", e); return 1; }
            }
        }
        Err(e) => { println!("curl: socket failed: {}", e); return 1; }
    }
}
