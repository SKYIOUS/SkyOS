...
fn resolve_host(host: &str, port: u16) -> Result<i64, ()> {
    ...
    if parts.len() == 4 && parts.iter().all(|p| p.parse::<u8>().is_ok()) {
        let ip = [parts[0].parse().unwrap(), parts[1].parse().unwrap(), parts[2].parse().unwrap(), parts[3].parse().unwrap()];
        let fd = net::socket(net::AF_INET, net::SOCK_STREAM, 0).map_err(|_| ())?;
        let addr = net::SockAddrIn::new(ip, port);
        net::connect(fd, addr.as_bytes()).map_err(|_| { let _ = io::close(fd); })?;
        return Ok(fd);
    }
    let mut ip = [0u8; 4];
...
fn user_main() -> i32 {
    ...
    let rest = match url.strip_prefix("http://") {
        Some(r) => r,
        None => { println!("wget: only http:// supported"); return 1; }
    };
    ...
    let (host, port) = if host_str.starts_with('[') {
        ...
        let p = if let Some(r) = rest_after.strip_prefix(':') { r.parse().unwrap_or(80) } else { 80 };
        (addr, p)
    } else {
        let parts: Vec<&str> = host_str.split(':').collect();
        let h = parts[0];
        let p = if parts.len() > 1 { parts[1].parse().unwrap_or(80) } else { 80 };
        (h, p)
    };

    let filename = if path == "/" { "index.html" } else {
    ...
    let fd = if host_str.starts_with('[') {
        ...
        match net::parse_ipv6(raw) {
            Some(ip6) => {
                ...
                if fd < 0 { println!("wget: IPv6 socket failed"); return 1; }
                let addr = net::SockAddrIn6::new(ip6, port);
                if net::connect(fd, addr.as_bytes()).is_err() {
                    println!("wget: IPv6 connect failed"); let _ = io::close(fd); return 1;
                }
                fd
            }
            None => { println!("wget: invalid IPv6 address"); return 1; }
        }
    } else {
        match resolve_host(host, port) {
            Ok(fd) => fd,
            Err(_) => { println!("wget: could not resolve {}", host); return 1; }
        }
    };
    ...
    if response.len() < 12 { println!("wget: empty response"); return 1; }
    ...
    if status_code == 0 || status_code >= 400 {
        ...
        println!("wget: HTTP {} {}", status_code, status_parts.get(2).unwrap_or(&""));
        println!("{}", body);
        return if status_code == 0 { 1 } else { status_code as i32 };
    }
    ...
    if let Some(pos) = response.windows(4).position(|w| w == b"\r\n\r\n") {
        ...
        if filename.ends_with(".html") || filename == "index.html" {
            ...
            match io::open(filename, 0x42) { // O_CREAT | O_RDWR
                Ok(out_fd) => {
