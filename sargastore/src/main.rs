...
impl Cat {
    ...
    fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "Utilities" => Some(Cat::Utilities),
            "Development" => Some(Cat::Development),
            "Games" => Some(Cat::Games),
            "Libraries" => Some(Cat::Libraries),
            _ => None,
        }
    }
}
...
fn defaults() -> Vec<Pkg> {
    vec![
        ...
        Pkg {
            name: "nettools",
            version: "1.0.0",
            desc: "Network utilities and tools",
            cat: Cat::System,
            installed: true,
        },
        Pkg {
                ...
                if !sec.is_empty() {
                    for p in pkgs.iter_mut() {
                        if p.name == sec.as_str() {
                            if !version.is_empty() {
                                p.version = leak_str(version);
                            }
                            if !desc.is_empty() {
                                p.desc = leak_str(desc);
                            }
                            if let Some(c) = cat {
                                p.cat = c;
                            }
...
fn user_main() -> i32 {
    ...
    let mut win = match Window::create("Sarga Store", W, H) {
        Ok(w) => w,
        Err(e) => {
            io::print_str(&alloc::format!("sargastore: window failed: {}\n", e));
            return 1;
        }
    };
    ...
    loop {
        ...
        let qs = if display_query.is_empty() {
            "Search packages... (press /)"
        } else {
            display_query.as_str()
        };
        let qc = if display_query.is_empty() {
            theme.text_disabled
        } else {
            theme.text
        };
        ...
        for (rank, &idx) in vis.iter().enumerate() {
            ...
            if y_full + CARD_H < LIST_TOP + scroll {
                continue;
            }
            if y_full > LIST_TOP + usable_h + scroll {
                break;
            }
            let y = y_full.saturating_sub(scroll);
            if y + CARD_H > LIST_BOT {
                continue;
            }
            let card_bg = if rank % 2 == 0 {
                theme.bg_surface
            } else {
                theme.bg_elevated
            };
            ...
            win.draw_string(
                CARD_X + 12 + p.name.len() as u32 * 8 + 12,
                y + 10,
                ver.as_str(),
            ...
            let cat_color = match p.cat {
                Cat::System => theme.accent_light,
                Cat::Utilities => theme.success,
                Cat::Development => theme.warning,
                Cat::Games => theme.accent,
                Cat::Libraries => theme.text_secondary,
                Cat::All => theme.text_secondary,
            };
