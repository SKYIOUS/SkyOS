...
use libsarga::io::{self, close, openpty, read, write};
...
const ANSI_COLORS: [u32; 16] = [
    0xFF000000, 0xFFCC0000, 0xFF00CC00, 0xFFCCCC00, 0xFF0000CC, 0xFFCC00CC, 0xFF00CCCC, 0xFFCCCCCC,
...
impl Terminal {
    ...
    fn tab(&mut self) {
        ...
        if new_col < COLS {
            self.cursor = self.row() * COLS + new_col;
        }
    }
}
...
impl Vt100Parser {
    ...
    fn process_csi(&self, term: &mut Terminal) {
        ...
        match final_byte {
                    ...
                    while i < n {
                        match p[i] {
                            ...
                            30..=37 => {
                                ...
                                if term.bold {
                                    c = (c & 0xFF000000) | (c & 0x00FFFFFF).wrapping_mul(2);
                                }
                                term.current_fg = c;
                            }
                            39 => term.current_fg = term.default_fg,
                            40..=47 => {
                                term.current_bg = ANSI_COLORS[(p[i] - 40) as usize];
                            }
                            ...
                            100..=107 => {
                                term.current_bg = ANSI_COLORS[(p[i] - 100 + 8) as usize];
                            }
                            _ => {}
                        }
                        i += 1;
            ...
            b'H' | b'f' => {
                let row = if n > 0 {
                    (p[0] as usize).saturating_sub(1)
                } else {
                    0
                };
                let col = if n > 1 {
                    (p[1] as usize).saturating_sub(1)
                } else {
                    0
                };
            ...
            b'A' => {
                ...
                term.set_pos(term.row().saturating_sub(n), term.col());
            }
            b'B' => {
                let n = if n > 0 { p[0] as usize } else { 1 };
                term.set_pos((term.row() + n).min(ROWS - 1), term.col());
            }
            b'C' => {
                let n = if n > 0 { p[0] as usize } else { 1 };
                term.set_pos(term.row(), (term.col() + n).min(COLS - 1));
            }
            b'D' => {
                let n = if n > 0 { p[0] as usize } else { 1 };
                term.set_pos(term.row(), term.col().saturating_sub(n));
            }
            b'E' => {
                let n = if n > 0 { p[0] as usize } else { 1 };
                term.set_pos(term.row() + n, 0);
            }
            b'F' => {
                let n = if n > 0 { p[0] as usize } else { 1 };
                term.set_pos(term.row().saturating_sub(n), 0);
            }
            b'G' => {
                let col = if n > 0 {
                    (p[0] as usize).saturating_sub(1)
                ...
                term.set_pos(term.row(), col);
            }
            b'J' => {
                let mode = if n > 0 { p[0] as u8 } else { 0 };
            ...
            b'd' => {
                ...
                term.set_pos(row.min(ROWS - 1), term.col());
            }
            b'r' => {
                let top = if n > 0 {
                    (p[0] as usize).saturating_sub(1)
        ...
        match self.state {
            ...
            ParserState::Escape => {
                ...
                match byte {
                    ...
                    b'M' => {
                        if term.row() > 0 {
                            term.set_pos(term.row() - 1, term.col());
                        }
                        self.state = ParserState::Ground;
                    }
                    b'D' => {
                        if term.col() > 0 {
                            term.set_pos(term.row(), term.col() - 1);
                        }
                        self.state = ParserState::Ground;
                    }
                    b'E' => {
            ...
            ParserState::CsiParam => {
                ...
                match byte {
                    b'0'..=b'9' => {
                        ...
                        *param = (*param) * 10 + (byte - b'0') as u16;
                    }
                    b';' => {
                        self.param_count += 1;
...
fn user_main() -> i32 {
    ...
    let mut win = match Window::create("sarga-term", win_w, win_h) {
        Ok(w) => w,
        Err(e) => {
            io::print_str(&alloc::format!("sarga-term: window failed: {}\n", e));
            return 0;
        }
    };
    ...
    loop {
        ...
        loop {
            ...
            if mouse.buttons == 0 && mouse.scroll == 0 {
                break;
            }
            ...
            if mouse.buttons & 1 == 0 && mouse.buttons & 2 == 0 && mouse.scroll == 0 {
                break;
            }
        }
