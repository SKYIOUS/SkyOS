...
use alloc::vec::Vec;
use libsarga::{
    gui::Window, io, process, sarga_main, textbox::TextBox, theme::Theme, widget::Widget,
};
...
static CATALOG: &[PkgInfo] = &[
    PkgInfo {
        name: "System Monitor",
        version: "1.0.0",
        description: "View CPU, memory and process information",
        depends: &[],
    },
    PkgInfo {
        name: "File Manager",
    ...
    PkgInfo {
        name: "Terminal",
        version: "2.0.0",
        description: "Command-line shell and terminal emulator",
        depends: &[],
    },
    PkgInfo {
        name: "Calculator",
        version: "1.2.0",
        description: "Simple yet powerful calculator",
        depends: &[],
    },
    PkgInfo {
        name: "Clock",
        version: "1.0.0",
        description: "World clock with alarms and timers",
        depends: &[],
    },
    PkgInfo {
        name: "Notes",
    ...
    PkgInfo {
        name: "Paint",
        version: "1.0.0",
        description: "Simple drawing and image editing tool",
        depends: &[],
    },
    PkgInfo {
        name: "SargaEdit",
    ...
    PkgInfo {
        name: "SargaView",
        version: "1.0.0",
        description: "Image and document viewer",
        depends: &[],
    },
    PkgInfo {
        name: "SargaBuild",
        version: "0.9.0",
        description: "Build system and task runner",
        depends: &[],
    },
    PkgInfo {
        name: "SkyScript",
        version: "0.5.0",
        description: "SkyOS scripting language runtime",
        depends: &[],
    },
    PkgInfo {
        name: "KorLang IDE",
        version: "0.2.0",
        description: "IDE for KorLang development",
        depends: &["SkyScript"],
    },
    PkgInfo {
        name: "SargaPlayer",
    ...
    PkgInfo {
        name: "SargaDocs",
        version: "0.6.0",
        description: "Word processor and document editor",
        depends: &[],
    },
    PkgInfo {
        name: "SargaSheets",
    ...
    PkgInfo {
        name: "Calendar",
        version: "1.0.0",
        description: "Calendar and schedule manager",
        depends: &[],
    },
];
...
fn list_dir(path: &str) -> Vec<String> {
    ...
    loop {
        ...
        while off + 19 <= n as usize {
            ...
            let name_end = name_start
                + dent_buf[name_start..off + reclen]
                    .iter()
                    .position(|&b| b == 0)
                    .unwrap_or(reclen - 19);
    ...
    unsafe {
        libsarga::syscall::syscall1(3, fd as u64);
    }
    entries
}
...
fn filter_packages(pkgs: &[Package], query: &str) -> Vec<usize> {
    if query.is_empty() {
        return (0..pkgs.len()).collect();
    }
    let q = ascii_lower(query);
    pkgs.iter()
        .enumerate()
        .filter(|(_, p)| {
            ascii_lower(&p.name).contains(&q) || ascii_lower(&p.description).contains(&q)
        })
        .map(|(i, _)| i)
        .collect()
}
                    ...
                    if let View::Browse = view {
                        if !search.is_focused() {
                            ...
                            if !fi.is_empty() {
                                ...
                                let vis = 7;
                                if sel >= scroll as usize + vis {
                                    scroll = (sel - vis + 1) as u32;
                                }
                            }
                        }
                    }
                        ...
                        if (10..690).contains(&mx) && my >= ey as i32 && my < (ey + 50) as i32 {
                                ...
                                && (my as u32) < ey + 40
                            {
                                if packages[pi].installed {
                                    ...
                                    let path =
                                        alloc::format!("/packages/{}.skp", packages[pi].name);
                                    show_progress(
                                        &mut win,
                                        &theme,
                                        &alloc::format!("Installing {}...", packages[pi].name),
                                    );
                                    run_install(&path);
                        ...
                        if packages[idx].installed {
                            ...
                            if show_confirm(&mut win, &theme, &msg) {
                                show_progress(
                                    &mut win,
                                    &theme,
                                    &alloc::format!("Removing {}...", packages[idx].name),
                                );
                            ...
                            show_progress(
                                &mut win,
                                &theme,
                                &alloc::format!("Installing {}...", packages[idx].name),
                            );
                            run_install(&path);
                            packages = discover_packages();
                        }
