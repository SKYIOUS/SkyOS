...
pub(crate) fn draw_clipboard(win: &mut libsarga::gui::Window, snap: &RenderSnapshot) {
...
pub(crate) fn draw_switcher(win: &mut libsarga::gui::Window, snap: &RenderSnapshot) {
    ...
    win.draw_rect_alpha(0, 0, snap.screen_w, snap.screen_h, 0x40000000);
    win.draw_rounded_rect(bx, by, bw, bh, 8, 0xFF2D2D2D);
    win.draw_rounded_rect_outline(bx, by, bw, bh, 8, 0xFF555555);
...
pub(crate) fn draw_context_menu(win: &mut libsarga::gui::Window, snap: &RenderSnapshot) {
    if let Some((mx, my, items)) = snap.context_menu {
        let mw = 150u32;
        let mh = items.len() as u32 * 28 + 10;
        win.draw_rounded_rect(mx as u32, my as u32, mw, mh, 6, snap.theme.bg_elevated);
        win.draw_rounded_rect_outline(mx as u32, my as u32, mw, mh, 6, snap.theme.border);
        for (i, (name, _)) in items.iter().enumerate() {
            let iy = my as u32 + 5 + i as u32 * 28;
            win.draw_string(mx as u32 + 10, iy + 6, name, snap.theme.text, 0);
        }
    }
}
