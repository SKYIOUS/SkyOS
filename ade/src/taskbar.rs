...
pub(crate) fn draw(win: &mut Window, snap: &RenderSnapshot, clock_str: &str) {
    ...
    let th = snap.theme;
    win.draw_gradient_rect(0, ty, snap.screen_w, TASKBAR_H, th.bg_surface, th.bg_primary, true);
    win.draw_line_h(0, ty, snap.screen_w, th.border);

    let start_hover = Rect::new(5, ty as i32 + 4, 58, TASKBAR_H - 8)
        .hit_test(snap.mouse);
    ...
    for (i, aw) in snap.windows.iter().enumerate() {
        ...
        let hover = Rect::new(bx as i32, ty as i32 + 4, 120, TASKBAR_H - 8)
            .hit_test(snap.mouse);
