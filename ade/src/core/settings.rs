...
impl SettingsState {
    ...
    pub fn draw(&self, canvas: &mut Canvas, snap: &RenderSnapshot) {
        ...
        canvas.draw_string(px + 10, py + 6, "Settings", 0xFFFFFFFF, 0);
        ...
        for (i, (label, val)) in rows.iter().enumerate() {
            ...
            canvas.draw_rounded_rect(px + 8, iy, pw - 16, 28, 4, bg);
            canvas.draw_string(px + 16, iy + 6, label, 0xFFD0D0D0, 0);
        ...
        canvas.draw_rounded_rect(px + 100, py + ph - 36, 120, 28, 4, cb);
        canvas.draw_string(px + 140, py + ph - 30, "Close", 0xFFD0D0D0, 0);
    }
