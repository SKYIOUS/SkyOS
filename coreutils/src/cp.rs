...
fn copy_recursive(src: &str, dst: &str) -> i64 {
    ...
    unsafe {
        syscall::syscall3(83, dst.as_ptr() as u64, 0o755, 0);
    }
        ...
        while i + 16 <= entries.len() {
            ...
            if namelen > 0 && i + 11 + namelen <= entries.len() {
                if let Ok(name) = core::str::from_utf8(&entries[i + 11..i + 11 + namelen]) {
                    ...
                    let mut st = [0u64; 32];
                    if unsafe {
                        syscall::syscall2(4, src_path.as_ptr() as u64, st.as_mut_ptr() as u64)
                    } == 0
                    {
...
fn user_main() -> i32 {
    ...
    for src in &files {
        let mut st = [0u64; 32];
        let is_dir = unsafe { syscall::syscall2(4, src.as_ptr() as u64, st.as_mut_ptr() as u64) }
            == 0
        ...
        if preserve {
            let mut st = [0u64; 32];
            unsafe {
                syscall::syscall2(4, src.as_ptr() as u64, st.as_mut_ptr() as u64);
            }
        }
    }
    0
