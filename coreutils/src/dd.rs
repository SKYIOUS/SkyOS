...
fn user_main() -> i32 {
    ...
    for i in 1..args::argc() {
        if let Some(arg) = args::get(i as usize) {
            if let Some(val) = arg.strip_prefix("if=") {
                ifile = String::from(val);
            } else if let Some(val) = arg.strip_prefix("of=") {
                ofile = String::from(val);
            } else if let Some(val) = arg.strip_prefix("bs=") {
                bs = val.parse().unwrap_or(512);
            } else if let Some(val) = arg.strip_prefix("count=") {
                count = val.parse().unwrap_or(usize::MAX);
            }
        }
    }
    let infd = if ifile.is_empty() {
        0
    } else {
        io::open(&ifile, 0).unwrap_or(0)
    };
    let outfd = if ofile.is_empty() {
        1
    } else {
        io::open(&ofile, 1).unwrap_or(1)
    };
    ...
    if ifile.is_empty() && infd != 0 {
        let _ = io::close(infd);
    }
    if !ofile.is_empty() {
        let _ = io::close(outfd);
    }
