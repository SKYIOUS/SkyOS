...
fn user_main() -> i32 {
    ...
    let fd = match io::open(dev_path, 2) {
        Ok(f) => f,
        Err(e) => {
            println!("Error: cannot open {}: {}", dev_path, e);
            return 0;
        }
    };
    ...
    let sb_bytes = unsafe {
        core::slice::from_raw_parts(
            &superblock as *const SkyfsSuperblock as *const u8,
    ...
    let inode_bytes = unsafe {
        core::slice::from_raw_parts(
            &root_inode as *const SkyfsInode as *const u8,
    ...
    while skip > 0 {
        ...
        if io::write(fd, &zero_buf[..to_write]).is_err() {
            break;
        }
    ...
    inode_buf[inode_block_offset..inode_block_offset + inode_bytes.len()]
    ...
    io::close(fd).ok();
    println!(
        "SkyFS filesystem created on {}: {} blocks, {} inodes",
        dev_path, total_blocks, inode_count
    );
    0
}
