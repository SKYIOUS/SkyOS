...
use alloc::vec::Vec;
use libsarga::{sarga_main, print, println, args, io, fs};

fn user_main() -> i32 {
    let fd = match io::open("/dev", 0) {
        Ok(fd) => fd,
        Err(_) => { println!("blkid: no block devices available"); return 0; }
    };
    ...
    loop {
        ...
        while off < n {
            ...
            let reclen = u16::from_ne_bytes(buf[off+16..off+18].try_into().unwrap_or([0;2])) as usize;
    ...
    for (ino, mode, name) in &devices {
        let ftype = if mode & 0o170000 == 0o060000 { "block" }
                    else if mode & 0o170000 == 0o020000 { "char" }
                    else if mode & 0o170000 == 0o040000 { "dir" }
                    else { "unknown" };
