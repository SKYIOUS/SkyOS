...
fn execute_lines(input: &str, ctx: &mut ScriptContext) -> i64 {
    ...
    let mut i = 0;
    while i < lines.len() {
        if ctx.break_flag || ctx.continue_flag {
            break;
        }
...
fn execute_if(lines: &[&str], start: usize, ctx: &mut ScriptContext) -> usize {
    ...
    while i < lines.len() && !lines[i].trim().starts_with("fi") {
        i += 1;
    }
    if i < lines.len() {
        i + 1
...
fn execute_while(lines: &[&str], start: usize, ctx: &mut ScriptContext) -> (usize, bool) {
    ...
    let rest = if is_until {
        first.strip_prefix("until ").unwrap_or("")
    } else {
        first.strip_prefix("while ").unwrap_or("")
    };
...
fn execute_case(lines: &[&str], start: usize, ctx: &mut ScriptContext) -> usize {
    ...
    while i < lines.len() {
        ...
        if line.ends_with(')') {
            ...
            let matches = pattern
                .split('|')
                .any(|p| glob_match(p.trim(), &expanded_word));
            if matches {
                matched = true;
            }
            i += 1;
    ...
    while i < lines.len() && !lines[i].trim().starts_with("esac") {
        i += 1;
    }
    i + 1
}
            ...
            match chars[pos + 1] {
                ...
                c if c.is_digit(10) => {
                    ...
                    let val = if idx == 0 {
                        ...
                        ctx.pos_args.get(idx - 1).cloned().unwrap_or_default()
                    };
                    out.push_str(&val);
                    pos += 2;
                }
                ...
                '?' => {
                    out.push_str(&alloc::format!("{}", crate::get_last_exit()));
                    pos += 2;
                }
                '$' => {
                    pos += 2;
