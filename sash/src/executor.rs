use crate::parser::{Command, Connector, Pipeline, RedirectKind};
...
fn execute_pipeline(commands: &[Command], bg: bool) -> i64 {
    ...
    if n == 1 {
        return execute_command(&commands[0], None, None, bg);
    }
    ...
    for (i, cmd) in commands.iter().enumerate() {
        ...
        if pid == 0 {
            ...
            if let Some(fd) = stdin {
                unsafe {
                    libsarga::syscall::syscall2(33, fd as u64, 0);
                }
                let _ = unsafe { libsarga::syscall::syscall1(3, fd as u64) };
            }
            if let Some(fd) = stdout {
                unsafe {
                    libsarga::syscall::syscall2(33, fd as u64, 1);
                }
                let _ = unsafe { libsarga::syscall::syscall1(3, fd as u64) };
            }
    ...
    for redir in &cmd.redirects {
        match redir.kind {
            RedirectKind::Out | RedirectKind::Append => {
                let flags = if redir.kind == RedirectKind::Append {
                    0x401
                } else {
                    0x241
                };
...
pub fn capture_output(cmd_str: &str) -> String {
    ...
    let pipelines = crate::parser::parse(&tokens);
    if pipelines.is_empty() || pipelines[0].commands.is_empty() {
        return String::new();
    }
